...
def plot_cumulative_below_threshold(
    ...
    fig, ax = plt.subplots(figsize=(REVTEX_COLUMN_WIDTH, REVTEX_COLUMN_WIDTH * 0.65), layout="tight")
    ...
    for i, ri_factor in enumerate(sorted_ri):
        ...
        for element, points in element_data.items():
            matching = [
                p for p in points
                if abs(p["ri_factor"] - ri_factor) < ri_factor_tol
            ]
        ...
        for rc in sorted(all_rc_values):
            ...
            if n_with_data == 0:
                continue
            n_ok = sum(
                1 for el_data in element_rc_ok.values()
                if el_data.get(rc, False)
            )
            rc_plot.append(rc)
            fractions.append(n_ok / n_with_data)

        ax.fill_between(rc_plot, fractions, color=colors[i],
                        label=f"$r_i/r_c = {ri_factor:.1f}$")
