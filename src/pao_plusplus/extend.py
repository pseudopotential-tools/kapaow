...
from pao_plusplus.basis import PseudoatomicBasis, AtomicBasis, ordered_subshells, Subshell, AngularMomentum
...
class BasisExtensionViaAddition(BasisExtension):
    ...
    def extend(self, basis: AtomicBasis | PseudoatomicBasis) -> PseudoatomicBasis:
        if isinstance(basis, PseudoatomicBasis):
            raise TypeError("Cannot extend pseudoatomic bases by addition because we can't know what l channel to add to")
...
class BasisExtensionViaPolarization(BasisExtension):
    ...
    increment: int = Field(default=1, description="number of polarization orbitals to add per angular momentum channel")
