...
def test_atomic_basis_creation(atomic_basis: AtomicBasis) -> None:
    """Test the AtomicBasis class."""
    assert atomic_basis.l_max == AngularMomentum.P
    assert atomic_basis.n_max == 2


def test_atomic_basis_extend(atomic_basis: AtomicBasis) -> None:
...
def test_atomic_basis_to_pseudoatomic_basis(atomic_basis: AtomicBasis) -> None:
    ...
    assert pseudo_basis.n_max == 1  # note: 1, not 2, because we don't have n = 1
    # orbitals in the pseudo basis
